...
class move_to_see:
    ...
    def execute(self):
        ...
        self.start_pose = self.interface.getCurrentPose()
        self.start_pixels = self.x_ref.pixel_size
        
        while (self.avg_abs_gradient > self.tolerance) and (self.x_ref.pixel_size < self.max_pixel) and (self.count < 125):

            # raw_input("Press Enter to continue...")
            
            # delta_matrix [3 x 3] contains the differences in objective function from the reference camera.
            delta_matrix, self.x_ref, self.x, self.ref_size_no_noise, self.x_ref_obj_val = self.getNumericalDerivatives(self.noise_std,use_noise=True)
            # delta_flat [8] is delta_matrix as a row vector with the ref camera removed.
            delta_flat = np.delete(delta_matrix.reshape((1,9)),4,None) # <-- PZ can remove

            #print ('Numerical deltas: \n')
            #print (delta_matrix)
            #print "\n"
            
            # uses objective function differences from 8 peripheral cameras to determine there is any
            # fruit present,  and if not then abort the sim.
            # <-- PZ can be replaced with: if self.x_ref.pixel_size == 0 then abort
            if (np.sum(delta_matrix) == 0.0):
                print ("delta matrix = 0")
                break
            else:
                
                # <-- PZ can remove, as this is only used to help calc directional grad, which will be replaced by CNN
                numerical_derivative = np.divide(delta_flat,self.camera_vector_mags)
                
                # <-- PZ update with:
                # remove: self.computeDirDerviative()
                # get image from reference camera
                # transform image to tensor
                # input temsor image to CNN to get predicted target values = self.gradient
                self.gradient = self.computeDirDerivative(self.camera_unit_vectors,numerical_derivative)
                self.gradients.append(self.gradient)    # keep a record of gradient for each timestep
                ...
                self.avg_abs_gradient = sum(self.avg_abs_gradient_queue)/self.queue_size # calc average of last 5 gradients
                self.abs_gradient = np.linalg.norm(self.gradient)                       # get L2 norm of current gradient
                self.avg_abs_gradient_queue.appendleft(self.abs_gradient)               # put current gradient in queue
                self.avg_abs_gradient_queue.pop()                                       # remove last gradient in queue
                
                pose_delta = self.step_size*self.gradient # <-- x,y,z calculated first from gradient [1 x 3]
                ...
                if self.interface.type == "ROS":
                    ...
                    if self.set_orientation==True:
                        pose_delta = np.append(pose_delta,-dPitch/2) #add 0 angle for euler x <-- PZ add pitch to back of pose delta [1 x 4]
                        pose_delta = np.append(pose_delta,-dRoll/2)  # <-- PZ add roll to back of pose delta [1 x 5]
                    else:
                        pose_delta = np.append(pose_delta,0) 
                        pose_delta = np.append(pose_delta,0) 
                    
                    pose_delta = np.append(pose_delta,0) # <-- PZ add yaw to back of pose delta [1 x 6]
                    pose_delta = pose_delta.reshape((6,)) # <-- PZ not sure what this accomplishes
                    ...
                    self.images.append(self.interface.getImage())  #PZ Added
        ...
        ret_dict['start_pixels'] = self.start_pixels
        ret_dict['start_pose'] = self.start_pose
        
        ret_dict['images'] = self.images  #PZ Added
        
        return ret_dict
    
    def execute_CNN(self, CNN_model, device):

        # define transform so that VREP sim image can be input to CNN
        ...
        while (self.avg_abs_gradient > self.tolerance) and (self.x_ref.pixel_size < self.max_pixel) and (self.count < 125):
            ...
            if (np.sum(delta_matrix) == 0.0):
                ...
                # input temsor image to CNN to get predicted target values = self.gradient
                #self.gradient = self.computeDirDerivative(self.camera_unit_vectors,numerical_derivative)
                ref_image = self.interface.getImage()
